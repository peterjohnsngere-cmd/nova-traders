...
/*
 * Convert the current digit counts into
 * whole-number percentages that always
 * add up to exactly 100%.
 *
 * We only display the distribution once
 * the full 100-tick sample is available.
 */
 ...
 * MARKET DATA
 *
 * Load the latest 100 historical ticks first.
 * Then subscribe to live ticks and maintain
 * the rolling 100-tick distribution.
 */
useEffect(() => {
    ...
    const subscribe = async () => {
        try {
            ...
            /*
             * One-time historical request.
             *
             * subscribe is intentionally omitted
             * so this request cannot accidentally
             * become the live tick subscription.
             */
            ...
            /*
             * Start live ticks after the historical
...
/*
 * LIVE PROPOSAL PAYOUTS
 *
 * Requests the current payout for each
 * contract button whenever the market,
 * stake, duration, barrier or contract mode
 * changes.
 */
useEffect(() => {
    if (
    ...
    /*
     * Small debounce so changing the stake
     * does not send a request for every single
     * keystroke.
     */
